...
var count = 0; // Draw if reach to 9
...
function insertSymbol(element, symbol) {
  ...
  if(win || turn > 8) {
    $("#restart").addClass("reset-btn");  // Highlights "restart" button
...
function checkWinConditions(element) {
  ...
  if(win) {
    ...
    showWinnerMsg(element.innerHTML);
...
function showWinnerMsg(winner){
	$("#show-winner").text(winner + " win...!!!")
}